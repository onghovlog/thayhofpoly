...
import { getCourseById, updateCourse, getYoutubeVideoInfo, getYoutubePlaylistInfo } from '@/services/courseService';
...
export default function EditCoursePage() {
      ...
      .then(([catsRes, courseRes]) => {
        ...
        if (courseRes.success && courseRes.data) {
          ...
          setFormData({
            ...
            duration: c.duration || 'Tự học theo tiến độ',
          ...
          setError(courseRes.message || 'Không tìm thấy khóa học');
        }
      })
      .catch((err) => {
  ...
  const handleAddSingleVideo = async () => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        const updatedLessons = [...formData.lessons, newLesson];
        const autoDur = calculateTotalDuration(updatedLessons);
  ...
  const handleLoadFromPlaylist = async () => {
    ...
    try {
      ...
      if (res.success && res.data?.items?.length > 0) {
        ...
        const autoDur = calculateTotalDuration(loadedLessons);
  ...
  const handleAddManualLesson = () => {
    ...
    const updated = [...formData.lessons, newLesson];
    setFormData((prev) => ({
      ...prev,
      duration: calculateTotalDuration(updated),
      lessons: updated,
    }));
  };
  ...
  const handleLessonChange = (index, field, value) => {
    ...
    const autoDur = field === 'duration' ? calculateTotalDuration(updated) : formData.duration;
    setFormData((prev) => ({ ...prev, duration: autoDur, lessons: updated }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setError(res.message || 'Không thể cập nhật khóa học.');
      }
    } catch (err) {
      ...
      >
        <div>
          ...
          <h1 style={{ fontSize: '1.85rem', color: 'var(--secondary)' }}>
            Chỉnh Sửa Khóa Học: {formData.title}
          </h1>
        </div>

        {formData.slug && (
          <Link
            href={`/khoa-hoc/${formData.slug}`}
            target="_blank"
