...
import { getLearningPathById, updateLearningPath } from '@/services/learningPathService';
...
export default function EditLearningPathPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([pathRes, catRes, courseRes, artRes]) => {
        ...
        if (pathRes.success && pathRes.data) {
            ...
            steps:
              p.steps && p.steps.length > 0
                ? p.steps.map((s, idx) => ({
                    ...
                    estimatedTime: s.estimatedTime || '1-2 tuần',
                    course: s.course?._id || s.course || '',
                    article: s.article?._id || s.article || '',
          ...
          setError(pathRes.message || 'Không tìm thấy lộ trình');
        }
      })
      .catch((err) => {
        setError(err.message || 'Lỗi khi tải thông tin lộ trình');
      })
      .finally(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await updateLearningPath(pathId, payload);
      if (res.success) {
        setSuccessMsg('Cập nhật lộ trình học tập thành công!');
        setTimeout(() => {
          router.push('/admin/learning-paths');
        }, 1000);
      } else {
        setError(res.message || 'Không thể cập nhật lộ trình.');
      }
    } catch (err) {
      ...
      >
        <div>
          ...
          <h1 style={{ fontSize: '1.85rem', color: 'var(--secondary)' }}>
            Chỉnh Sửa Lộ Trình: {formData.title}
          </h1>
        </div>

        {formData.slug && (
          <Link
            href={`/lo-trinh/${formData.slug}`}
            target="_blank"
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1.25rem' }}>
            ...
            <div className="form-group">
              ...
              <span style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginTop: '0.25rem', display: 'block' }}>
                Đường dẫn trên web: /lo-trinh/{formData.slug}
              </span>
            </div>
